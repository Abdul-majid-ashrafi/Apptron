...
export class HomeComponent extends Component {
    ...
    isOnMSg(id, arg) {
        var chatMessages = [];
        console.log(id, this.state.user.uid);
        const chats = this.state.chat;
        for (let i = 0; i < chats.length; i++) {
            const obj = chats[i];
            if (obj.senderUid === id && obj.reciverID === this.state.user.uid) {
                chatMessages.push(obj);
            }
        }
    ...
    componentWillMount() {
        ...
        // var chatRef = firebase.database().ref('chatMessages/');
        // chatRef.orderByChild("senderUid")
        //     .equalTo(currentUID)
            ...
            <div>
                ...
                <ul>
                    {this.state.chat.map((val, idx) => {
                        return <li key={idx} onClick={this.isOnMSg.bind(this, val.senderUid)}>  <b>{val.sender}:</b> {val.message}  </li>
                    })}
                </ul>
